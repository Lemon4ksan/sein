...
func initDistanceParams(params *encoderParams, npostfix, ndirect uint32) {
	var (
		dist_params   *distanceParams = &params.dist
	...
	if params.large_window {
		var (
			bound          = [maxNpostfix + 1]uint32{0, 4, 12, 28}
...
func recomputeDistancePrefixes(cmds []command, orig_params, new_params *distanceParams) {
...
func computeDistanceCost(cmds []command, orig_params, new_params *distanceParams, cost *float64) bool {
	var (
		equal_params bool = false
		dist_prefix  uint16
		dist_extra   uint32
		extra_bits   float64 = 0.0
		histo        histogramDistance
	)
...
func buildMetaBlock(
	...
	var (
		distance_histograms        []histogramDistance
		literal_histograms         []histogramLiteral
		literal_context_modes      []int = nil
		literal_histograms_size    uint
		distance_histograms_size   uint
		i                          uint
		literal_context_multiplier uint = 1
		npostfix                   uint32
		ndirect_msb                uint32        = 0
		check_orig                 bool          = true
		best_dist_cost             float64       = 1e99
		orig_params                encoderParams = *params
	)
	...
	for npostfix = 0; npostfix <= maxNpostfix; npostfix++ {
		for ; ndirect_msb < 16; ndirect_msb++ {
			var (
				ndirect   uint32 = ndirect_msb << npostfix
				skip      bool
				dist_cost float64
			)
...
func initContextBlockSplitter(
	...
	alphabet_size, num_contexts, min_block_size uint,
	...
	var (
		max_num_blocks uint = num_symbols/min_block_size + 1
		max_num_types  uint
	)
...
func contextBlockSplitterFinishBlock(self *contextBlockSplitter, is_final bool) {
	var (
		split        *blockSplit        = self.split_
		num_contexts uint               = self.num_contexts_
		last_entropy []float64          = self.last_entropy_[:]
		histograms   []histogramLiteral = self.histograms_
	)
	...
	if self.num_blocks_ == 0 {
		...
		var (
			entropy          [maxStaticContexts]float64
			combined_histo   []histogramLiteral = make([]histogramLiteral, (2 * num_contexts))
			combined_entropy [2 * maxStaticContexts]float64
			diff             = [2]float64{0.0}
		)
		...
		for i = 0; i < num_contexts; i++ {
			var (
				curr_histo_ix uint = self.curr_histogram_ix_ + i
				j             uint
			)
			...
			for j = 0; j < 2; j++ {
				var (
					jx                uint = j*num_contexts + i
					last_histogram_ix uint = self.last_histogram_ix_[j] + i
				)
		...
		if split.num_types < self.max_block_types_ && diff[0] > self.split_threshold_ &&
...
*/
func contextBlockSplitterAddSymbol(self *contextBlockSplitter, symbol, context uint) {
...
func mapStaticContexts(num_contexts uint, static_context_map []uint32, mb *metaBlockSplit) {
	...
	for i = 0; i < mb.literal_split.num_types; i++ {
		var (
			offset uint32 = uint32(i * num_contexts)
			j      uint
		)
...
func buildMetaBlockGreedyInternal(
	...
	var (
		...
		cmd_blocks   blockSplitterCommand
		dist_blocks  blockSplitterDistance
		num_literals uint = 0
	)
...
func optimizeHistograms(num_distance_codes uint32, mb *metaBlockSplit) {
	var (
		good_for_rle [numCommandSymbols]byte
		i            uint
	)
