...
func combineLengthCodes(inscode, copycode uint16, use_last_distance bool) uint16 {
...
func getLengthCode(insertlen, copylen uint, use_last_distance bool, code *uint16) {
	var (
		inscode  uint16 = getInsertLengthCode(insertlen)
		copycode uint16 = getCopyLengthCode(copylen)
	)
...
func commandRestoreDistanceCode(self *command, dist *distanceParams) uint32 {
	if uint32(self.dist_prefix_&0x3FF) < numDistanceShortCodes+dist.num_direct_distance_codes {
		...
		var (
			dcode        uint32 = uint32(self.dist_prefix_) & 0x3FF
			nbits        uint32 = uint32(self.dist_prefix_) >> 10
			extra        uint32 = self.dist_extra_
			postfix_mask uint32 = (1 << dist.distance_postfix_bits) - 1
			hcode        uint32 = (dcode - dist.num_direct_distance_codes - numDistanceShortCodes) >> dist.distance_postfix_bits
			lcode        uint32 = (dcode - dist.num_direct_distance_codes - numDistanceShortCodes) & postfix_mask
			offset       uint32 = ((2 + (hcode & 1)) << nbits) - 4
		)
...
func commandDistanceContext(self *command) uint32 {
	var (
		r uint32 = uint32(self.cmd_prefix_) >> 6
		c uint32 = uint32(self.cmd_prefix_) & 7
	)
...
func commandCopyLenCode(self *command) uint32 {
	var (
		modifier uint32 = self.copy_len_ >> 25
		delta    int32  = int32(int8(byte(modifier | (modifier&0x40)<<1)))
	)
