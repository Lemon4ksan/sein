...
func decodeWindowBits(s *Reader, br *bitReader) int {
	var (
		n            uint32
		large_window bool = s.large_window
	)
...
func safeDecodeSymbol(table []huffmanCode, br *bitReader, result *uint32) bool {
	var (
		val            uint32
		available_bits uint32 = getAvailableBits(br)
	)
	if available_bits == 0 {
		if table[0].bits == 0 {
...
func readSimpleHuffmanSymbols(alphabet_size, max_symbol uint32, s *Reader) int {
	var (
		br       *bitReader = &s.br
		max_bits uint32     = log2Floor(alphabet_size - 1)
		i        uint32     = s.sub_loop_counter
	)
...
func processRepeatedCodeLength(
	...
	*repeat += repeat_delta + 3
	...
	if *repeat_code_len != 0 {
		var (
			last uint = uint(*symbol + repeat_delta)
			next int  = next_symbol[*repeat_code_len]
		)
		for {
...
func readSymbolCodeLengths(alphabet_size uint32, s *Reader) int {
	var (
		br                *bitReader = &s.br
		symbol            uint32     = s.symbol
		repeat            uint32     = s.repeat
		space             uint32     = s.space
		prev_code_len     uint32     = s.prev_code_len
		repeat_code_len   uint32     = s.repeat_code_len
		symbol_lists      symbolList = s.symbol_lists
		code_length_histo []uint16   = s.code_length_histo[:]
		next_symbol       []int      = s.next_symbol[:]
	)
...
func safeReadSymbolCodeLengths(alphabet_size uint32, s *Reader) int {
	...
	for s.symbol < alphabet_size && s.space > 0 {
		...
		if code_len < repeatPreviousCodeLength {
			...
			var (
				extra_bits   uint32 = code_len - 14
				repeat_delta uint32 = (bits >> p[0].bits) & bitMask(extra_bits)
			)
...
func readCodeLengthCodeLengths(s *Reader) int {
	var (
		br        *bitReader = &s.br
		num_codes uint32     = s.repeat
		space     uint32     = s.space
		i         uint32     = s.sub_loop_counter
	)
	for ; i < codeLengthCodes; i++ {
		var (
			code_len_idx byte = kCodeLengthCodeOrder[i]
			ix           uint32
			v            uint32
		)
...
func readHuffmanCode(alphabet_size, max_symbol uint32, table []huffmanCode, opt_table_size *uint32, s *Reader) int {
	...
	for {
		...
		case stateHuffmanComplex:
			{
				var (
					i      uint32
					result int = readCodeLengthCodeLengths(s)
				)
				if result != decoderSuccess {
					return result
				}
		...
		case stateHuffmanLengthSymbols:
			var (
				table_size uint32
				result     int = readSymbolCodeLengths(max_symbol, s)
			)
...
func huffmanTreeGroupDecode(group *huffmanTreeGroup, s *Reader) int {
	...
	for s.htree_index < int(group.num_htrees) {
		var (
			table_size uint32
			result     int = readHuffmanCode(
				uint32(group.alphabet_size),
				uint32(group.max_symbol),
				s.next,
				&table_size,
				s,
			)
...
func decodeContextMap(context_map_size uint32, num_htrees *uint32, context_map_arg *[]byte, s *Reader) int {
	...
	case stateContextMapDecode:
		{
			var (
				context_index         uint32 = s.context_index
				max_run_length_prefix uint32 = s.max_run_length_prefix
				context_map           []byte = *context_map_arg
				code                  uint32 = s.code
				skip_preamble         bool   = (code != 0xFFFF)
			)
...
func decodeBlockTypeAndLength(safe int, s *Reader, tree_type int) bool {
	...
	var (
		br         *bitReader = &s.br
		ringbuffer []uint32   = s.block_type_rb[tree_type*2:]
		block_type uint32
	)
...
func detectTrivialLiteralBlockTypes(s *Reader) {
	...
	for i = 0; uint32(i) < s.num_block_types[0]; i++ {
		var (
			offset uint = i << literalContextBits
			error  uint = 0
			sample uint = uint(s.context_map[offset])
			j      uint
		)
...
func prepareLiteralDecoding(s *Reader) {
	var (
		context_mode   byte
		trivial        uint
		block_type     uint32 = s.block_type_rb[1]
		context_offset uint32 = block_type << literalContextBits
	)
...
func writeRingBuffer(s *Reader, available_out *uint, next_out *[]byte, total_out *uint, force bool) int {
	...
	var (
		to_write    uint = unwrittenBytes(s, true)
		num_written uint = *available_out
	)
...
func calculateRingBufferSize(s *Reader) {
	var (
		window_size         int = 1 << s.window_bits
		new_ringbuffer_size int = window_size
		min_size            int
	)
...
func readDistanceInternal(safe int, s *Reader, br *bitReader) bool {
	var (
		...
		distance_tree []huffmanCode = []huffmanCode(s.distance_hgroup.htrees[s.dist_htree_index])
	)
	if safe == 0 {
...
func readCommandInternal(safe int, s *Reader, br *bitReader, insert_length *int) bool {
	...
	*insert_length = int(v.insert_len_offset)
	if safe == 0 {
		...
		if !safeReadBitsMaybeZero(br, uint32(v.insert_len_extra_bits), &insert_len_extra) ||
			!safeReadBitsMaybeZero(br, uint32(v.copy_len_extra_bits), &copy_length) {
			bitReaderRestoreState(br, &memento)
			return false
		}
	}
...
CommandInner:
	...
	if s.trivial_literal_context != 0 {
		...
		var (
			p1 byte = s.ringbuffer[(pos-1)&s.ringbuffer_mask]
			p2 byte = s.ringbuffer[(pos-2)&s.ringbuffer_mask]
		)
		for {
			var context byte
...
CommandPostDecodeLiterals:
	...
	if s.distance_code > s.max_distance {
		...
		if i >= minDictionaryWordLength && i <= maxDictionaryWordLength {
			var (
				address       int         = s.distance_code - s.max_distance - 1
				words         *dictionary = s.dictionary
				trans         *transforms = s.transforms
				offset        int         = int(s.dictionary.offsets_by_length[i])
				shift         uint32      = uint32(s.dictionary.size_bits_by_length[i])
				mask          int         = int(bitMask(shift))
				word_idx      int         = address & mask
				transform_idx int         = address >> shift
			)
...
func decoderDecompressStream(
	...
	for {
		...
		case stateHuffmanCode1:
			{
				var (
					alphabet_size uint32 = s.num_block_types[s.loop_counter] + 2
					tree_offset   int    = s.loop_counter * huffmanMaxSize258
				)
		...
		case stateHuffmanCode2:
			{
				var (
					...
					tree_offset   int    = s.loop_counter * huffmanMaxSize26
				)
		...
		case stateContextMap1:
			result = decodeContextMap(
				s.num_block_types[0]<<literalContextBits,
		...
		case stateContextMap2:
			{
				var (
					num_direct_codes    uint32 = s.num_direct_distance_codes - numDistanceShortCodes
				...
				if s.large_window {
					num_distance_codes = uint32(
						distanceAlphabetSize(
							uint(s.distance_postfix_bits),
							uint(num_direct_codes),
					...
					num_distance_codes = uint32(
						distanceAlphabetSize(uint(s.distance_postfix_bits), uint(num_direct_codes), maxDistanceBits),
					)
				...
				result = decodeContextMap(
					s.num_block_types[2]<<distanceContextBits,
					&s.num_dist_htrees,
					&s.dist_context_map,
					s,
				)
