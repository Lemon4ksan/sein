	...
	if distance_code < numDistanceShortCodes+num_direct_codes {
		...
		*extra_bits = 0
		...
		var (
			...
			bucket       uint = uint(log2FloorNonZero(dist) - 1)
			postfix_mask uint = (1 << postfix_bits) - 1
			postfix      uint = dist & postfix_mask
			prefix       uint = (dist >> bucket) & 1
			offset       uint = (2 + prefix) << bucket
			nbits        uint = bucket - postfix_bits
		)
