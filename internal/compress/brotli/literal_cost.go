...
func estimateBitCostsForLiteralsUTF8(pos, len, mask uint, data []byte, cost []float32) {
	...
	var (
		...
		window_half    uint = 495
		in_window      uint = brotli_min_size_t(window_half, uint(len))
	...
	for i = 0; i < len; i++ {
		...
		if i+window_half < len {
			var (
				c      uint = uint(data[(pos+i+window_half-1)&mask])
				last_c uint = uint(data[(pos+i+window_half-2)&mask])
			)
...

		{
			...
			var (
				utf8_pos   uint = utf8Position(last_c, c, max_utf8)
				masked_pos uint = (pos + i) & mask
				histo      uint = histogram[utf8_pos][data[masked_pos]]
				lit_cost   float64
			)
