...
func initBlockSplitterCommand(
	self *blockSplitterCommand,
	...
	var (
		...
		max_num_types  uint = brotli_min_size_t(max_num_blocks, maxNumberOfBlockTypes+1)
	)
...
func blockSplitterFinishBlockCommand(self *blockSplitterCommand, is_final bool) {
	var (
		...
		histograms   []histogramCommand = self.histograms_
	)
	...
	if self.num_blocks_ == 0 {
		...
		var (
			entropy          float64 = bitsEntropy(histograms[self.curr_histogram_ix_].data_[:], self.alphabet_size_)
			combined_histo   [2]histogramCommand
			combined_entropy [2]float64
			diff             [2]float64
			j                uint
		)
		for j = 0; j < 2; j++ {
