...
func initialEntropyCodesDistance(data []uint16, length, stride, num_histograms uint, histograms []histogramDistance) {
	var (
		seed         uint32 = 7
...
func randomSampleDistance(seed *uint32, data []uint16, length, stride uint, sample *histogramDistance) {
	var pos uint = 0
	if stride >= length {
		stride = length
	} else {
...
func refineEntropyCodesDistance(data []uint16, length, stride, num_histograms uint, histograms []histogramDistance) {
	var (
...
func clusterBlocksDistance(data []uint16, length, num_blocks uint, block_ids []byte, split *blockSplit) {
	var (
		...
		all_histograms          []histogramDistance = make([]histogramDistance, all_histograms_capacity)
		...
		num_clusters            uint                = 0
		histograms              []histogramDistance = make(
			[]histogramDistance,
