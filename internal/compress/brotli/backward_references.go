...
func computeDistanceCode(distance, max_distance uint, dist_cache []int) uint {
	if distance <= max_distance {
		var (
			distance_plus_3 uint = distance + 3
			offset0         uint = distance_plus_3 - uint(dist_cache[0])
			offset1         uint = distance_plus_3 - uint(dist_cache[1])
		)
...
func createBackwardReferences(
	...
	var (
		...
		insert_length      uint = *last_insert_len
		pos_end            uint = position + num_bytes
		store_end          uint
	)
	...
	var (
		random_heuristics_window_size uint = literalSpreeLengthForSparseSearch(params)
		apply_random_heuristics       uint = position + random_heuristics_window_size
		gap                           uint = 0
	)
	...
	for position+hasher.HashTypeLength() < pos_end {
		var (
			max_length   uint = pos_end - position
			max_distance uint = brotli_min_size_t(position, max_backward_limit)
		)
		...
		if sr.score > kMinScore {
			...
			max_distance = brotli_min_size_t(position, max_backward_limit)
			{
				...
				*commands = append(
					*commands,
					makeCommand(&params.dist, insert_length, sr.len, sr.len_code_delta, distance_code),
				)
			}
			...
			*num_literals += insert_length
			...
			{
				var (
					range_start uint = position + 2
					range_end   uint = brotli_min_size_t(position+sr.len, store_end)
				)
				if sr.distance < sr.len>>2 {
					range_start = brotli_min_size_t(
						range_end,
						brotli_max_size_t(range_start, position+sr.len-(sr.distance<<2)),
					)
				}
			...
			if position > apply_random_heuristics {
				...
				if position > apply_random_heuristics+4*random_heuristics_window_size {
					...
					var (
						kMargin  uint = brotli_max_size_t(hasher.StoreLookahead()-1, 2)
						pos_jump uint = brotli_min_size_t(position+8, pos_end-kMargin)
					)
