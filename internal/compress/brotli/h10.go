...
func (h *h10) Prepare(one_shot bool, input_size uint, data []byte) {
	var (
		invalid_pos uint32 = h.invalid_pos_
		i           uint32
	)
	for i = 0; i < 1<<17; i++ {
...
func storeAndFindMatchesH10(
	...
	var (
		...
		max_comp_len       uint     = brotli_min_size_t(max_length, 128)
		should_reroot_tree bool     = (max_length >= 128)
		key                uint32   = hashBytesH10(data[cur_ix_masked:])
		forest             []uint32 = self.forest
		prev_ix            uint     = uint(self.buckets_[key])
		node_left          uint     = leftChildIndexH10(self, cur_ix)
		node_right         uint     = rightChildIndexH10(self, cur_ix)
		best_len_left      uint     = 0
		best_len_right     uint     = 0
		depth_remaining    uint
	)
	...
	for depth_remaining = 64; ; depth_remaining-- {
		var (
			...
			prev_ix_masked uint = prev_ix & ring_buffer_mask
		)
		...
		{
			var (
				cur_len uint = brotli_min_size_t(best_len_left, best_len_right)
				len     uint
			)
...
func findAllMatchesH10(
	...
	var (
		orig_matches             []backwardMatch = matches
		cur_ix_masked            uint            = cur_ix & ring_buffer_mask
		best_len                 uint            = 1
		short_match_max_backward uint
	)
	...
	var (
		stop         uint = cur_ix - short_match_max_backward
		dict_matches [maxStaticDictionaryMatchLen + 1]uint32
		i            uint
	)
...

	{
		...
		if findAllStaticDictionaryMatches(dictionary, data[cur_ix_masked:], minlen, max_length, dict_matches[0:]) {
			var (
				maxlen uint = brotli_min_size_t(maxStaticDictionaryMatchLen, max_length)
				l      uint
			)
...
func (h *h10) StitchToPreviousBlock(num_bytes, position uint, ringbuffer []byte, ringbuffer_mask uint) {
	if num_bytes >= h.HashTypeLength()-1 && position >= 128 {
		var (
			i_start uint = position - 128 + 1
			i_end   uint = brotli_min_size_t(position, i_start+num_bytes)
		)
