...
func testStaticDictionaryItem(
	...
	{
		var (
			cut          uint = len - matchlen
			transform_id uint = (cut << 2) + uint((dictionary.cutoffTransforms>>(cut*6))&0x3F)
		)
...
func searchInStaticDictionary(
	...
	var (
		key  uint
		i    uint
		self *hasherCommon = handle.Common()
	)
...
func hasherSetup(handle *hasherHandle, params *encoderParams, data []byte, position, input_size uint, is_last bool) {
	var (
		self     hasherHandle  = nil
		common   *hasherCommon = nil
		one_shot bool          = (position == 0 && is_last)
	)
