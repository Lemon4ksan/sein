...
func (f *framer) Append(
	...
	var (
		lastFrame      ackhandler.StreamFrame
		streamFrameLen protocol.ByteCount
	)
...
func (f *framer) getNextStreamFrame(
	...
	if f.nonIncrementalStreams[urgency].Empty() ||
		(!f.lastSendWasIncremental[urgency] && !f.incrementalStreams[urgency].Empty()) {
