...
func TestStreamFlowControllerFinalOffset(t *testing.T) {
	...
	t.Run("non-final offset past final offset", func(t *testing.T) {
		...
		require.Equal(
			t,
			"received offset 60 for stream 42, but final offset was already received at 50",
			terr.ErrorMessage,
		)
	})
	...
	t.Run("final offset smaller than previous offset", func(t *testing.T) {
		...
		require.Equal(
			t,
			"received final offset 40 for stream 42, but already received offset 50 before",
			terr.ErrorMessage,
		)
	})
}
