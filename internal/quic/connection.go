...
type unpacker interface {
	...
	UnpackShortHeader(
		...
		data []byte,
	) (protocol.PacketNumber, protocol.PacketNumberLen, protocol.KeyPhaseBit, []byte, error)
}
...
var deadlineSendImmediately = monotime.Time(
	42 * time.Millisecond,
) // any value > time.Time{} and before time.Now() is fine
...
func (c *Conn) handleHandshakeComplete(now monotime.Time) error {
	...
	if ticket != nil { // may be nil if session tickets are disabled via tls.Config.SessionTicketsDisabled
		_, _ = c.oneRTTStream.Write(ticket)
...
func (c *Conn) handleOnePacket(rp receivedPacket) (wasProcessed bool, _ error) {
	...
	var (
		counter    uint8
		lastConnID protocol.ConnectionID
	)
...
func (c *Conn) handleShortHeaderPacket(
	...
	if err != nil {
		...
		wasQueued, err = c.handleUnpackError(err, p, protocol.PacketType(0))
	...
	isNonProbing, pathChallenge, err := c.handleUnpackedShortHeaderPacket(destConnID, pn, data, p.ecn, p.rcvTime)
	if err != nil {
		return false, err
	}
	...
	destConnID, frames, shouldSwitchPath := c.pathManager.HandlePacket(
		p.remoteAddr,
		p.rcvTime,
		pathChallenge,
		isNonProbing,
...
func (c *Conn) handleLongHeaderPacket(
	p receivedPacket,
	hdr *wire.Header,
) (wasProcessed bool, _ error) {
	var wasQueued bool
	...
	if c.receivedFirstPacket && hdr.Type == protocol.PacketTypeInitial && hdr.SrcConnectionID != c.handshakeDestConnID {
		c.logger.Debugf(
			"Dropping Initial packet (%d bytes) with unexpected source connection ID: %s (expected %s)",
			p.Size(),
			hdr.SrcConnectionID,
			c.handshakeDestConnID,
		)
	...
	if err != nil {
		wasQueued, err = c.handleUnpackError(err, p, hdr.Type)
		return false, err
	}
	...
	if err := c.handleUnpackedLongHeaderPacket(
		packet,
		p.ecn,
		p.rcvTime,
	); err != nil {
...
func (c *Conn) handleUnpackError(
	err error,
	p receivedPacket,
	pt protocol.PacketType,
) (wasQueued bool, _ error) {
	switch {
	...
	case errors.Is(err, wire.ErrInvalidReservedBits):
		return false, &qerr.TransportError{ErrorCode: qerr.ProtocolViolation, ErrorMessage: err.Error()}
...
func (c *Conn) handleRetryPacket(
	hdr *wire.Header,
	data []byte,
	rcvTime monotime.Time,
) bool /* was this a valid Retry */ {
...
func (c *Conn) handleUnpackedLongHeaderPacket(
	...
	isAckEliciting, _, _, err := c.handleFrames(
		packet.data,
	...
	return c.receivedPacketHandler.ReceivedPacket(
		packet.hdr.PacketNumber,
		ecn,
		packet.encryptionLevel,
		rcvTime,
...
func (c *Conn) handleUnpackedShortHeaderPacket(
	...
	isAckEliciting, isNonProbing, pathChallenge, err := c.handleFrames(
		data,
		destConnID,
	...
	if err := c.receivedPacketHandler.ReceivedPacket(
		pn,
		ecn,
		protocol.Encryption1RTT,
		rcvTime,
...
func (c *Conn) handleFrames(
	...
	for len(data) > 0 {
		...
		case frameType.IsAckFrameType():
			...
			if err := c.handleAckFrame(ackFrame, encLevel, rcvTime); err != nil {
				return false, false, nil, err
			}
...
func (c *Conn) handleCryptoFrame(
	frame *wire.CryptoFrame,
	encLevel protocol.EncryptionLevel,
	rcvTime monotime.Time,
) error {
...
func (c *Conn) restoreTransportParameters(params *wire.TransportParameters) {
	...
	c.peerParams = params
	_ = c.connIDGenerator.SetMaxActiveConnIDs(params.ActiveConnectionIDLimit)
...
func (c *Conn) handleTransportParameters(params *wire.TransportParameters) error {
	...
	if c.perspective == protocol.PerspectiveClient && c.peerParams != nil && c.ConnectionState().Used0RTT &&
		!params.ValidForUpdate(c.peerParams) {
		return &qerr.TransportError{
			ErrorCode:    qerr.ProtocolViolation,
...
func (c *Conn) applyTransportParameters() {
	...
	if params.PreferredAddress != nil {
		...
		_ = c.connIDManager.AddFromPreferredAddress(
			params.PreferredAddress.ConnectionID,
			params.PreferredAddress.StatelessResetToken,
		)
	}
...
func (c *Conn) sendPackets(now monotime.Time) error {
	if c.perspective == protocol.PerspectiveClient && c.handshakeConfirmed {
		if pm := c.pathManagerOutgoing.Load(); pm != nil {
			...
			if ok {
				...
				_, _ = tr.WriteTo(buf.Data, c.conn.RemoteAddr())
	...
	if !c.handshakeConfirmed {
		...
		if err := c.sendPackedCoalescedPacket(
			packet,
			c.sentPacketHandler.ECNMode(packet.IsOnlyShortHeaderPacket()),
			now,
		); err != nil {
			return err
		}
...
func (c *Conn) appendOneShortHeaderPacket(
	buf *packetBuffer,
...
func (c *Conn) registerPackedShortHeaderPacket(p shortHeaderPacket, ecn protocol.ECN, now monotime.Time) {
	...
	if c.firstAckElicitingPacketAfterIdleSentTime.IsZero() &&
		(len(p.StreamFrames) > 0 || ackhandler.HasAckElicitingFrames(p.Frames)) {
...
func (c *Conn) tryQueueingUndecryptablePacket(
	p receivedPacket,
	pt protocol.PacketType,
) {
	_ = pt
