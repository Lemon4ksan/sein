...
func testStreamsMapHandleReceiveStreamFrames(
	t *testing.T,
	pers protocol.Perspective,
	handleFrame func(*streamsMap, protocol.StreamID) error,
) {
	mockCtrl := gomock.NewController(t)
	mockSender := NewMockStreamSender(mockCtrl)
		...
		t.Run(pers.String(), func(t *testing.T) {
			...
			t.Run("MAX_STREAM_DATA frame", func(t *testing.T) {
				testStreamsMapHandleSendStreamFrames(t,
					pers,
					func(m *streamsMap, id protocol.StreamID) error {
						return m.HandleMaxStreamDataFrame(
							&wire.MaxStreamDataFrame{StreamID: id, MaximumStreamData: 1000},
						)
					},
				)
			})
...
func testStreamsMapHandleSendStreamFrames(
	t *testing.T,
	pers protocol.Perspective,
	handleFrame func(m *streamsMap, id protocol.StreamID) error,
) {
	mockCtrl := gomock.NewController(t)
	mockSender := NewMockStreamSender(mockCtrl)
