...
func (h *ExtendedHeader) GetLength(_ protocol.Version) protocol.ByteCount {
	length := 1 /* type byte */ + 4 /* version */ + 1 /* dest conn ID len */ + protocol.ByteCount(
		h.DestConnectionID.Len(),
	) + 1 /* src conn ID len */ + protocol.ByteCount(
...
func (h *ExtendedHeader) Log(logger utils.Logger) {
	...
	if h.Type == protocol.PacketTypeInitial || h.Type == protocol.PacketTypeRetry {
		if len(h.Token) == 0 {
			//nolint:gosec // G101: Logging QUIC token field, not hardcoded credentials.
			token = "Token: (empty), "
		} else {
		...
		if h.Type == protocol.PacketTypeRetry {
			logger.Debugf(
				"\tLong Header{Type: %s, DestConnectionID: %s, SrcConnectionID: %s, %sVersion: %s}",
				h.Type,
				h.DestConnectionID,
				h.SrcConnectionID,
				token,
	...
	logger.Debugf(
		"\tLong Header{Type: %s, DestConnectionID: %s, SrcConnectionID: %s, %sPacketNumber: %d, PacketNumberLen: %d, Length: %d, Version: %s}",
		h.Type,
		h.DestConnectionID,
		h.SrcConnectionID,
		token,
