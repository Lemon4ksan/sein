...
func TestLogCryptoFrame(t *testing.T) {
	...
	require.Contains(
		t,
		buf.String(),
		"\t<- &wire.CryptoFrame{Offset: 42, Data length: 123, Offset + Data length: 165}\n",
	)
}
...
func TestLogStreamFrame(t *testing.T) {
	...
	require.Contains(
		t,
		buf.String(),
		"\t<- &wire.StreamFrame{StreamID: 42, Fin: false, Offset: 1337, Data length: 100, Offset + Data length: 1437}\n",
	)
}
...
func TestLogNewConnectionIDFrame(t *testing.T) {
	...
	require.Contains(
		t,
		buf.String(),
		"\t<- &wire.NewConnectionIDFrame{SequenceNumber: 42, RetirePriorTo: 24, ConnectionID: deadbeef, StatelessResetToken: 0x0102030405060708090a0b0c0d0e0f10}",
	)
}
