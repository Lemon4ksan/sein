...
func ParseVersionNegotiationPacket(
	b []byte,
) (dest, src protocol.ArbitraryLenConnectionID, _ []protocol.Version, _ error) {
...
func ComposeVersionNegotiation(
	...
	expectedLen := 1 /* type byte */ + 4 /* version field */ + 1 /* dest connection ID length field */ + destConnID.Len() + 1 /* src connection ID length field */ + srcConnID.Len() + len(
		greasedVersions,
	)*4
