...
func LogFrame(logger utils.Logger, frame Frame, sent bool) {
	...
	case *CryptoFrame:
		...
		logger.Debugf(
			"\t%s &wire.CryptoFrame{Offset: %d, Data length: %d, Offset + Data length: %d}",
			dir,
			f.Offset,
			dataLen,
			f.Offset+dataLen,
	...
	case *StreamFrame:
		logger.Debugf(
			"\t%s &wire.StreamFrame{StreamID: %d, Fin: %t, Offset: %d, Data length: %d, Offset + Data length: %d}",
			dir,
			f.StreamID,
			f.Fin,
			f.Offset,
	...
	case *ResetStreamFrame:
		logger.Debugf(
			"\t%s &wire.ResetStreamFrame{StreamID: %d, ErrorCode: %#x, FinalSize: %d}",
			dir,
			f.StreamID,
			f.ErrorCode,
			f.FinalSize,
	...
	case *AckFrame:
		...
		if len(f.AckRanges) > 1 {
			...
			logger.Debugf(
				"\t%s &wire.AckFrame{LargestAcked: %d, LowestAcked: %d, AckRanges: {%s}, DelayTime: %s%s}",
				dir,
				f.LargestAcked(),
				f.LowestAcked(),
				strings.Join(ackRanges, ", "),
				f.DelayTime.String(),
				ecn,
			)
		} else {
			logger.Debugf(
				"\t%s &wire.AckFrame{LargestAcked: %d, LowestAcked: %d, DelayTime: %s%s}",
				dir,
				f.LargestAcked(),
				f.LowestAcked(),
				f.DelayTime.String(),
	...
	case *MaxStreamDataFrame:
		logger.Debugf(
			"\t%s &wire.MaxStreamDataFrame{StreamID: %d, MaximumStreamData: %d}",
			dir,
			f.StreamID,
			f.MaximumStreamData,
		)
	...
	case *StreamDataBlockedFrame:
		logger.Debugf(
			"\t%s &wire.StreamDataBlockedFrame{StreamID: %d, MaximumStreamData: %d}",
			dir,
			f.StreamID,
			f.MaximumStreamData,
		)
	...
	case *NewConnectionIDFrame:
		logger.Debugf(
			"\t%s &wire.NewConnectionIDFrame{SequenceNumber: %d, RetirePriorTo: %d, ConnectionID: %s, StatelessResetToken: %#x}",
			dir,
			f.SequenceNumber,
			f.RetirePriorTo,
			f.ConnectionID,
			f.StatelessResetToken,
		)
