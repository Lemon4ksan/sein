...
func TestShortHeaderLen(t *testing.T) {
	require.Equal(
		t,
		protocol.ByteCount(8),
		ShortHeaderLen(protocol.ParseConnectionID([]byte{1, 2, 3, 4}), protocol.PacketNumberLen3),
	)
	require.Equal(
		t,
		protocol.ByteCount(2),
		ShortHeaderLen(protocol.ParseConnectionID([]byte{}), protocol.PacketNumberLen1),
	)
}
...
func TestLogShortHeaderWithConnectionID(t *testing.T) {
	...
	require.Contains(
		t,
		buf.String(),
		"Short Header{DestConnectionID: deadbeefcafe1337, PacketNumber: 1337, PacketNumberLen: 4, KeyPhase: 1}",
	)
}
