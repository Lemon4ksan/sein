...
func (h *receivedPacketHistory) ReceivedPacket(
	p protocol.PacketNumber,
) bool /* is a new packet (and not a duplicate / delayed packet) */ {
...
func (h *receivedPacketHistory) addToRanges(
	p protocol.PacketNumber,
) bool /* is a new packet (and not a duplicate / delayed packet) */ {
	if len(h.ranges) == 0 {
