...
func testSentPacketHandlerSendAndAcknowledge(t *testing.T, encLevel protocol.EncryptionLevel) {
	...
	require.Equal(
		t,
		[]protocol.PacketNumber{pns[0], pns[1], pns[2], pns[3], pns[4], pns[7], pns[8], pns[9]},
		packets.Acked,
	)
...
func TestSentPacketHandlerRTTAckEliciting(t *testing.T) {
	...
	getPacketsInFlight := func() int {
		...
		sh := sph.(*sentPacketHandler)
		if sh.initialPackets != nil {
			count += sh.initialPackets.history.numOutstanding
		}

		if sh.handshakePackets != nil {
			count += sh.handshakePackets.history.numOutstanding
		}

		if sh.appDataPackets != nil {
			count += sh.appDataPackets.history.numOutstanding
		}

		return count
	}
	getBytesInFlight := func() int {
		return int(sph.(*sentPacketHandler).bytesInFlight)
	}
...
func testSentPacketHandlerPTO(t *testing.T, encLevel protocol.EncryptionLevel, ptoMode SendMode) {
	...
	require.Equal(t, uint32(0), sph.(*sentPacketHandler).ptoCount)
...
func TestSentPacketHandlerPacketNumberSpacesPTO(t *testing.T) {
	...
	var (
		initialPNs, handshakePNs     [4]protocol.PacketNumber
		initialTimes, handshakeTimes [4]monotime.Time
	)
...
func testSentPacketHandlerRetry(t *testing.T, rtt, expectedRTT time.Duration) {
	...
	for range 2 {
		...
		sph.SentPacket(
			...
			protocol.InvalidPacketNumber,
			nil,
			[]Frame{initialPackets.NewPingFrame(pn)},
			protocol.EncryptionInitial,
			protocol.ECNNon,
			1000,
			false,
		...
		sph.SentPacket(
			...
			protocol.InvalidPacketNumber,
			nil,
			[]Frame{appDataPackets.NewPingFrame(pn)},
...
func TestSentPacketHandlerRetryAfterPTO(t *testing.T) {
	...
	sph.SentPacket(
		...
		protocol.InvalidPacketNumber,
		nil,
		[]Frame{packets.NewPingFrame(pn1)},
		protocol.EncryptionInitial,
		protocol.ECNNon,
		1000,
		false,
	...
	sph.SentPacket(
		...
		protocol.InvalidPacketNumber,
		nil,
		[]Frame{packets.NewPingFrame(pn2)},
		protocol.EncryptionInitial,
		protocol.ECNNon,
		900,
		false,
...
func TestSentPacketHandlerECN(t *testing.T) {
	...
	sph.SentPacket(
		monotime.Now(),
		sph.PopPacketNumber(protocol.EncryptionHandshake),
		protocol.InvalidPacketNumber,
		nil,
		nil,
		protocol.EncryptionHandshake,
	...
	sph.SentPacket(
		monotime.Now(),
		sph.PopPacketNumber(protocol.Encryption0RTT),
		protocol.InvalidPacketNumber,
		nil,
		nil,
		protocol.Encryption0RTT,
	...
	ecnHandler.EXPECT().
		HandleNewlyAcked(gomock.Any(), int64(10), int64(11), int64(12)).
		DoAndReturn(func(packets []packetWithPacketNumber, _, _, _ int64) bool {
...
func TestSentPacketHandlerSpuriousLoss(t *testing.T) {
	...
	require.Equal(
		t,
		[]protocol.PacketNumber{pns[7], pns[8], pns[9], pns[10], pns[11], pns[13], pns[14], pns[15]},
		packets.Lost,
	)
}
