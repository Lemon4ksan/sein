...
import (
	...
	"github.com/lemon4ksan/sein/internal/quic/internal/congestion"
...
func newPacketNumberSpace(initialPN protocol.PacketNumber, isAppData bool) *packetNumberSpace {
	...
	if isAppData {
		pns = newSkippingPacketNumberGenerator(
			initialPN,
			protocol.SkipPacketInitialPeriod,
			protocol.SkipPacketMaxPeriod,
		)
	} else {
...
func NewSentPacketHandler(
	...
	if enableECN {
		h.enableECN = true
		h.ecnTracker = newECNTracker(logger)
	}
...
func (h *sentPacketHandler) DropPackets(encLevel protocol.EncryptionLevel, now monotime.Time) {
	...
	case protocol.Encryption0RTT:
		...
		for pn, p := range h.appDataPackets.history.Packets() {
			...
			h.removeFromBytesInFlight(p)
			_ = h.appDataPackets.history.Remove(pn)
		}
...
func (h *sentPacketHandler) ReceivedPacket(encLevel protocol.EncryptionLevel, rcvTime monotime.Time) {
...
func (h *sentPacketHandler) HasPendingPTO() bool {
	return h.numProbesToSend > 0
}
...
func (h *sentPacketHandler) SentPacket(
	...
	isPathMTUProbePacket, isPathProbe bool,
) {
	h.bytesSent += length
	h.connStats.BytesSent.Add(uint64(length))

	pnSpace := h.getPacketNumberSpace(encLevel)
	if pn <= pnSpace.largestSent {
		panic("non-sequential packet number")
	}
	...
	if isAckEliciting {
		h.bytesInFlight += length
		h.congestion.OnPacketSent(t, h.bytesInFlight, pn, length, true)

		if h.enableECN {
			h.ecnTracker.SentPacket(pn, ecn)
		}
	} else {
		h.congestion.OnPacketSent(t, h.bytesInFlight, pn, length, false)
	}
...
) (bool /* contained 1-RTT packet */, error) {
	pnSpace := h.getPacketNumberSpace(encLevel)
		...
		if ack.HasMissingRanges() {
			...
			if pn > ackRange.Largest {
				return nil, false, fmt.Errorf(
					"BUG: ackhandler would have acked wrong packet %d, while evaluating range %d -> %d",
					pn,
					ackRange.Smallest,
					ackRange.Largest,
				)
...
func (h *sentPacketHandler) getLossTimeAndSpace() (monotime.Time, protocol.EncryptionLevel) {
	...
	if h.handshakePackets != nil &&
		(lossTime.IsZero() || (!h.handshakePackets.lossTime.IsZero() && h.handshakePackets.lossTime.Before(lossTime))) {
...
func (h *sentPacketHandler) getPTOTimeAndSpace(
	now monotime.Time,
) (pto monotime.Time, encLevel protocol.EncryptionLevel) {
...
func (h *sentPacketHandler) setLossDetectionTimer(now monotime.Time) {
	h.alarm = h.lossDetectionTime(now)
}
...
func (h *sentPacketHandler) detectLostPackets(now monotime.Time, encLevel protocol.EncryptionLevel) {
	...
	for pn, p := range pnSpace.history.Packets() {
		...
		var packetLost bool
		switch {
		case !p.SendTime.After(lostSendTime):
			packetLost = true
		...
		case pnSpace.history.Difference(pnSpace.largestAcked, pn) >= packetThreshold:
			packetLost = true
		...
		case pnSpace.lossTime.IsZero():
			...
			if h.logger.Debug() {
				h.logger.Debugf(
					"\tsetting loss timer for packet %d (%s) to %s (in %s)",
					pn,
					encLevel,
					lossDelay,
					lossTime,
...
func (h *sentPacketHandler) OnLossDetectionTimeout(now monotime.Time) error {
	...
	if h.bytesInFlight == 0 && !h.peerCompletedAddressValidation {
		...
		default:
			return errors.New(
				"sentPacketHandler BUG: PTO fired, but bytes_in_flight is 0 and Initial and Handshake already dropped",
			)
		}
	...
	if !ps.history.HasOutstandingPackets() && !ps.history.HasOutstandingPathProbes() &&
		!h.peerCompletedAddressValidation {
		return nil
	}
...
func (h *sentPacketHandler) PeekPacketNumber(
	encLevel protocol.EncryptionLevel,
	...
	if numTrackedPackets >= protocol.MaxOutstandingSentPackets {
		if h.logger.Debug() {
			h.logger.Debugf(
				...
				numTrackedPackets,
				protocol.MaxOutstandingSentPackets,
			)
		}
