	...
	clientLoop:
		for {
			...
			case EventWriteInitialData:
				...
				if serverErr != nil {
					return clientEvents, clientErr, serverEvents, serverErr
				}
...
func TestHandlingNewSessionTicketFails(t *testing.T) {
	...
	require.True(t, func() *qerr.TransportError {
		target := &qerr.TransportError{}
		_ = errors.As(err, &target)
		return target
	}().ErrorCode.IsCryptoError())
}
...
func Test0RTT(t *testing.T) {
	...
	for _, ev := range serverEvents {
		if ev.Kind == EventReceivedReadKeys {
