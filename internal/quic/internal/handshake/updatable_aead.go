...
func (a *updatableAEAD) DecodePacketNumber(
...
func (a *updatableAEAD) open(
	...
	if kp != a.keyPhase.Bit() {
		if a.keyPhase > 0 && a.firstRcvdWithCurrentKey == protocol.InvalidPacketNumber ||
			pn < a.firstRcvdWithCurrentKey {
			if a.prevRcvAEAD == nil {
...
func (a *updatableAEAD) shouldInitiateKeyUpdate() bool {
	...
	if a.numRcvdWithCurrentKey >= keyUpdateInterval.Load() {
		a.logger.Debugf(
			"Received %d packets with current key phase. Initiating key update to the next key phase: %d",
			a.numRcvdWithCurrentKey,
			a.keyPhase+1,
		)
	...
	if a.numSentWithCurrentKey >= keyUpdateInterval.Load() {
		a.logger.Debugf(
			"Sent %d packets with current key phase. Initiating key update to the next key phase: %d",
			a.numSentWithCurrentKey,
			a.keyPhase+1,
		)
