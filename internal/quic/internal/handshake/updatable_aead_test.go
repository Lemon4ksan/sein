...
func setupEndpoints(
	...
	client = newUpdatableAEAD(utils.NewRTTStats(), utils.DefaultLogger, protocol.Version1)
	server = newUpdatableAEAD(serverRTTStats, utils.DefaultLogger, protocol.Version1)
...
func TestChaChaTestVector(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("QUIC %s", tc.version), func(t *testing.T) {
			...
			aead := newUpdatableAEAD(utils.NewRTTStats(), utils.DefaultLogger, tc.version)
			...
			packet := append(slices.Clone(header), payload...)
	...
	for _, v := range []protocol.Version{protocol.Version1, protocol.Version2} {
		for _, cs := range cipherSuites {
			t.Run(fmt.Sprintf("QUIC %s/%s", v, tls.CipherSuiteName(cs.ID)), func(t *testing.T) {
				...
				client := newUpdatableAEAD(utils.NewRTTStats(), utils.DefaultLogger, v)
				server := newUpdatableAEAD(utils.NewRTTStats(), utils.DefaultLogger, v)
	...
	for _, v := range []protocol.Version{protocol.Version1, protocol.Version2} {
		for _, cs := range cipherSuites {
			t.Run(fmt.Sprintf("QUIC %s/%s", v, tls.CipherSuiteName(cs.ID)), func(t *testing.T) {
				...
				client := newUpdatableAEAD(&rttStats, utils.DefaultLogger, v)
				server := newUpdatableAEAD(&rttStats, utils.DefaultLogger, v)
...
func TestUpdatableAEADPacketNumbers(t *testing.T) {
	client, server := setupEndpoints(t, utils.NewRTTStats())
	...
	require.Equal(
		t,
		protocol.PacketNumber(0x1337),
		server.FirstPacketNumber(),
	) // make sure we save the first packet number
...
func TestAEADLimitReached(t *testing.T) {
	client, _ := setupEndpoints(t, utils.NewRTTStats())
...
// 	client := newUpdatableAEAD(&rttStats, utils.DefaultLogger, protocol.Version1)
// 	server := newUpdatableAEAD(&rttStats, utils.DefaultLogger, protocol.Version1)
// 	client.SetReadKey(cs, trafficSecret2)
// 	client.SetWriteKey(cs, trafficSecret1)
// 	server.SetReadKey(cs, trafficSecret1)
// 	server.SetWriteKey(cs, trafficSecret2)
...
func TestDropsKeys3PTOsAfterKeyUpdate(t *testing.T) {
	...
	client, server := setupEndpoints(t, rttStats)
...
func TestInitiateKeyUpdateAfterSendingMaxPackets(t *testing.T) {
	const (
		firstKeyUpdateInterval = 5
		keyUpdateInterval      = 20
	)
...
func TestKeyUpdateEnforceACKKeyPhase(t *testing.T) {
	...
	_, server := setupEndpoints(t, utils.NewRTTStats())
	server.SetHandshakeConfirmed()
