	...
	if !r.hasMeasurement {
		...
		meanDev := time.Duration(
			oneMinusBeta*float32(
				...
				(smoothedRTT-sample).Abs()/time.Microsecond,
			),
		) * time.Microsecond
		newSmoothedRTT := time.Duration(
			(float32(smoothedRTT/time.Microsecond)*oneMinusAlpha)+(float32(sample/time.Microsecond)*rttAlpha),
		) * time.Microsecond
