...
func (s *HybridSlowStart) ShouldExitSlowStart(
	latestRTT, minRTT time.Duration,
	...
	if s.rttSampleCount == hybridStartMinSamples {
		...
		minRTTincreaseThreshold := time.Duration(
			max(minRTTincreaseThresholdUs, hybridStartDelayMinThresholdUs),
		) * time.Microsecond
