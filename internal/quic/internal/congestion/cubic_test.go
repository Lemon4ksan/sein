...
const (
	...
	nConnectionAlpha       float32 = 3 * float32(
		numConnections,
	) * float32(
		numConnections,
	) * (1 - nConnectionBeta) / (1 + nConnectionBeta)
	maxCubicTimeInterval = 30 * time.Millisecond
)

func renoCwnd(currentCwnd protocol.ByteCount) protocol.ByteCount {
	return currentCwnd + protocol.ByteCount(
		float32(maxDatagramSize)*nConnectionAlpha*float32(maxDatagramSize)/float32(currentCwnd),
	)
}
...
func TestCubicAboveOriginWithTighterBounds(t *testing.T) {
	...
	const (
		...
		rttMinS = float32(rttMin/time.Millisecond) / 1000.0
	)
