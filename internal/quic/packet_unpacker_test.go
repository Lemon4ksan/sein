...
func testUnpackLongHeaderPacket(t *testing.T,
	...
	data := append(slices.Clone(hdrRaw), payload...)
	...
	calls = append(
		...
		opener.EXPECT().
			...
			Return(protocol.PacketNumber(1234)),
...
func testUnpackShortHeaderPacket(
	t *testing.T,
	incorrectReservedBits bool,
	decryptResult decryptResult,
	expectedErr error,
...
func TestUnpackHeaderSampleShortHeader(t *testing.T) {
	...
	t.Run("minimal size", func(t *testing.T) {
		...
		opener.EXPECT().DecodePacketNumber(gomock.Any(), gomock.Any()).Return(protocol.PacketNumber(1337))
		opener.EXPECT().
			Open(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
...
func TestUnpackErrors(t *testing.T) {
	...
	opener.EXPECT().DecodePacketNumber(gomock.Any(), gomock.Any()).Return(protocol.PacketNumber(1234))
	opener.EXPECT().
		Open(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
		Return(nil, &qerr.TransportError{ErrorCode: qerr.CryptoBufferExceeded})
...
func TestUnpackHeaderDecryption(t *testing.T) {
	...
	gomock.InOrder(
		...
		opener.EXPECT().
			DecodePacketNumber(protocol.PacketNumber(0x1337), protocol.PacketNumberLen2).
			Return(protocol.PacketNumber(0x7331)),
		opener.EXPECT().
			Open(gomock.Any(), gomock.Any(), protocol.PacketNumber(0x7331), origHdrRaw).
			Return([]byte{0}, nil),
	)
