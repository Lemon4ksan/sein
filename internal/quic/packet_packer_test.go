...
import (
	...
	mockackhandler "github.com/lemon4ksan/aoni/x/quic/internal/mocks/ackhandler"
...
func newMockShortHeaderSealer(mockCtrl *gomock.Controller) *mocks.MockShortHeaderSealer {
	...
	sealer.EXPECT().
		Seal(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
		DoAndReturn(func(dst, src []byte, pn protocol.PacketNumber, associatedData []byte) []byte {
...
func testPackLongHeaders(t *testing.T, includeACK bool) {
	...
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.EncryptionInitial).
		Return(protocol.PacketNumber(0x24), protocol.PacketNumberLen3)
	tp.pnManager.EXPECT().PopPacketNumber(protocol.EncryptionInitial).Return(protocol.PacketNumber(0x24))
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.EncryptionHandshake).
		Return(protocol.PacketNumber(0x42), protocol.PacketNumberLen4)
...
func TestPackCoalescedAckOnlyPacketNothingToSend(t *testing.T) {
	...
	tp := newTestPacketPacker(t, mockCtrl, protocol.PerspectiveClient)
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.Encryption1RTT).
...
func TestPack0RTTPacket(t *testing.T) {
	...
	tp.ackFramer.EXPECT().GetAckFrame(protocol.EncryptionInitial, gomock.Any(), true)
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.Encryption0RTT).
		Return(protocol.PacketNumber(0x42), protocol.PacketNumberLen2)
...
func testPackCoalescedAppData(t *testing.T, withAck bool) {
	...
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.EncryptionHandshake).
		Return(protocol.PacketNumber(0x24), protocol.PacketNumberLen2)
...
func testPackConnectionCloseCoalesced(t *testing.T, pers protocol.Perspective) {
	...
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.EncryptionInitial).
		Return(protocol.PacketNumber(1), protocol.PacketNumberLen2)
	tp.pnManager.EXPECT().PopPacketNumber(protocol.EncryptionInitial).Return(protocol.PacketNumber(1))
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.EncryptionHandshake).
		Return(protocol.PacketNumber(2), protocol.PacketNumberLen2)
	...
	case protocol.PerspectiveClient:
		...
		tp.pnManager.EXPECT().
			PeekPacketNumber(protocol.Encryption0RTT).
			Return(protocol.PacketNumber(3), protocol.PacketNumberLen2)
		...
		[]ackhandler.Frame{
			{
				...
				Handler: &mtuFinderAckHandler{},
			}, // set any non-nil ackhandler.FrameHandler
...
func TestPackLargeDatagramFrame(t *testing.T) {
	...
	tp := newTestPacketPacker(t, mockCtrl, protocol.PerspectiveServer)
	tp.ackFramer.EXPECT().
		GetAckFrame(protocol.Encryption1RTT, gomock.Any(), true).
		Return(&wire.AckFrame{AckRanges: []wire.AckRange{{Largest: 100}}})
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.Encryption1RTT).
		Return(protocol.PacketNumber(0x42), protocol.PacketNumberLen2)
	tp.pnManager.EXPECT().PopPacketNumber(protocol.Encryption1RTT).Return(protocol.PacketNumber(0x42))
	...
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.Encryption1RTT).
		Return(protocol.PacketNumber(0x43), protocol.PacketNumberLen2)
	tp.sealingManager.EXPECT().Get1RTTSealer().Return(newMockShortHeaderSealer(mockCtrl), nil)
	tp.framer.EXPECT().HasData()
...
func packMaxNumNonAckElicitingAcks(
	t *testing.T,
	tp *testPacketPacker,
...
func TestPackLongHeaderPadToAtLeast4Bytes(t *testing.T) {
	...
	tp.pnManager.EXPECT().
		PeekPacketNumber(protocol.EncryptionHandshake).
		Return(protocol.PacketNumber(0x42), protocol.PacketNumberLen1)
	tp.pnManager.EXPECT().PopPacketNumber(protocol.EncryptionHandshake).Return(protocol.PacketNumber(0x42))
...
func TestPack1RTTProbePacket(t *testing.T) {
	...
	p, err := tp.packer.PackPTOProbePacket(
