	...
	if hasStreamWindowUpdate {
		...
		require.Equal(
			...
			&wire.MaxStreamDataFrame{
				StreamID:          streamID,
				MaximumStreamData: protocol.ByteCount(n) + streamReceiveWindow,
			},
			f.Frame,
		)
...
func testReceiveStreamDeadlineInThePast(
	t *testing.T,
	consumesBytes bool,
	op func(*ReceiveStream, []byte) (int, error),
) {
	mockFC := newTestStreamFlowController(42)
	str := newReceiveStream(42, nil, mockFC)
...
func testReceiveStreamPeekEOF(t *testing.T, longPeek bool) {
	synctest.Test(t, func(t *testing.T) {
		...
		require.NoError(
			t,
			str.handleStreamFrame(&wire.StreamFrame{Offset: 3, Data: []byte("bar"), Fin: true}, monotime.Now()),
		)
...
func TestReceiveStreamCancellation(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		require.NoError(
			t,
			str.handleResetStreamFrame(
				&wire.ResetStreamFrame{StreamID: 42, ErrorCode: 4321, FinalSize: 6},
				monotime.Now(),
			),
		)
...
func TestReceiveStreamResetAbandonsUnreadData(t *testing.T) {
	...
	require.NoError(
		t,
		str.handleResetStreamFrame(&wire.ResetStreamFrame{StreamID: streamID, ErrorCode: 1234, FinalSize: 6}, now),
	)
