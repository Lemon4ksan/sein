...
func TestFramerAppendStreamFrames(t *testing.T) {
	...
	controlFrames, fs, length = framer.Append(
		[]ackhandler.Frame{},
		[]ackhandler.StreamFrame{f0},
		protocol.MaxByteCount,
		monotime.Now(),
		protocol.Version1,
	)
...
func TestFramerPrioritizesStreamRetransmissions(t *testing.T) {
	...
	firstRetransmission := &wire.StreamFrame{
		StreamID:       firstRetransStreamID,
		Data:           []byte("first"),
		DataLenPresent: true,
	}
	secondRetransmission := &wire.StreamFrame{
		StreamID:       secondRetransStreamID,
		Data:           []byte("second"),
		DataLenPresent: true,
	}
	thirdRetransmission := &wire.StreamFrame{
		StreamID:       secondRetransStreamID,
		Offset:         6,
		Data:           []byte("third"),
		DataLenPresent: true,
	...
	str1.EXPECT().
		popRetransmissionFrame(gomock.Any(), protocol.Version1).
		Return(ackhandler.StreamFrame{Frame: firstRetransmission}, false)
	str2.EXPECT().
		popRetransmissionFrame(gomock.Any(), protocol.Version1).
		Return(ackhandler.StreamFrame{Frame: secondRetransmission}, true)
	str2.EXPECT().
		popRetransmissionFrame(gomock.Any(), protocol.Version1).
		Return(ackhandler.StreamFrame{Frame: thirdRetransmission}, false)
	newDataStr.EXPECT().
		popStreamFrame(gomock.Any(), protocol.Version1).
		Return(ackhandler.StreamFrame{Frame: newData}, nil, false)
...
func TestFramerSplitsStreamRetransmissions(t *testing.T) {
	...
	str1.EXPECT().
		popRetransmissionFrame(gomock.Any(), protocol.Version1).
		Return(ackhandler.StreamFrame{Frame: retransmit1}, false)
	str2.EXPECT().
		popRetransmissionFrame(gomock.Any(), protocol.Version1).
		DoAndReturn(func(maxSize protocol.ByteCount, v protocol.Version) (ackhandler.StreamFrame, bool) {
	...
	str2.EXPECT().
		popRetransmissionFrame(gomock.Any(), protocol.Version1).
		Return(ackhandler.StreamFrame{Frame: retransmit2}, false)
...
func newMockPriorityStream(
	t *testing.T,
	id protocol.StreamID,
	urgency int8,
	incremental bool,
	numFrames int,
) *MockStreamFrameGetter {
	t.Helper()
