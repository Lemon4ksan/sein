...
func TestSendStreamSetup(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), testCtxKey{}, "bar")
	str := newSendStream(ctx, 1337, nil, mockFC, false)
	require.NotNil(t, str.Context())
	require.Equal(t, "bar", str.Context().Value(testCtxKey{}))
...
func TestSendStreamPriorityGeneration(t *testing.T) {
	...
	str := newSendStream(
		...
		streamID,
		mockSender,
		newTestStreamFlowControllerWithSendWindow(streamID, protocol.MaxByteCount),
		false,
	)
...
func TestSendStreamSetReliableBoundaryAfterTryWriteAll(t *testing.T) {
	...
	require.Equal(
		t,
		&wire.ResetStreamFrame{StreamID: streamID, FinalSize: 100, ErrorCode: 42, ReliableSize: 100},
		cf.Frame,
	)
...
func TestSendStreamLargeWriteBlocking(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		errChan := make(chan error, 1)
		go func() {
			_, err := (&writerWithTimeout{Writer: str, Timeout: time.Second}).Write(
				make([]byte, protocol.MaxPacketBufferSize),
			)
			errChan <- err
		}()
...
func TestSendStreamCancellation(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		require.ErrorIs(
			...
			&StreamError{StreamID: streamID, ErrorCode: 1234, Remote: false},
		)
...
func TestSendStreamResetStreamAtRandomized(t *testing.T) {
	const (
		streamID protocol.StreamID = 123456
		dataLen                    = 8 << 10
	)
