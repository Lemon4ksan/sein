...
func (p *packetPacker) PackApplicationClose(
...
func (p *packetPacker) packConnectionClose(
	...
	encLevels := [4]protocol.EncryptionLevel{
	...
	for i, encLevel := range encLevels {
		...
		if encLevel == protocol.Encryption1RTT {
			shp, err := p.appendShortHeaderPacket(
				buffer,
				connID,
				oneRTTPacketNumber,
				oneRTTPacketNumberLen,
				keyPhase,
				payloads[i],
				0,
				maxPacketSize,
			...
			longHdrPacket, err := p.appendLongHeaderPacket(
				buffer,
				hdrs[i],
				payloads[i],
				paddingLen,
...
func (p *packetPacker) longHeaderPacketLength(
...
// It takes into account that packets that have a tiny payload need to be padded,
// such that len(payload) + packet number len >= 4 + AEAD overhead
func (p *packetPacker) shortHeaderPacketLength(
	connID protocol.ConnectionID,
...
func (p *packetPacker) initialPaddingLen(
	frames []ackhandler.Frame,
	currentSize, maxPacketSize protocol.ByteCount,
) protocol.ByteCount {
...
func (p *packetPacker) PackCoalescedPacket(
	...
	if (onlyAck && size == 0) || (!onlyAck && size < maxSize-protocol.MinCoalescedPacketSize) {
		...
		if handshakeSealer != nil {
			...
			if handshakePayload.length > 0 {
				s := p.longHeaderPacketLength(
					handshakeHdr,
					handshakePayload,
					v,
				) + protocol.ByteCount(
					handshakeSealer.Overhead(),
				)
				size += s
			}
		}
	...
	if initialPayload.length == 0 && handshakePayload.length == 0 && zeroRTTPayload.length == 0 &&
		oneRTTPayload.length == 0 {
		return nil, nil
	}
	...
	if initialPayload.length > 0 {
		...
		cont, err := p.appendLongHeaderPacket(
			buffer,
			initialHdr,
			initialPayload,
			padding,
...
func (p *packetPacker) PackAckOnlyPacket(
	maxSize protocol.ByteCount,
	now monotime.Time,
	v protocol.Version,
) (shortHeaderPacket, *packetBuffer, error) {
...
func (p *packetPacker) maybeGetCryptoPacket(
	...
	var (
		hasCryptoData  func() bool
		popCryptoFrame func(maxLen protocol.ByteCount) *wire.CryptoFrame
	)
...
func (p *packetPacker) composeNextPacket(
	...
	if hasRetransmission {
		for {
			...
			pl.frames = append(
				pl.frames,
				ackhandler.Frame{Frame: f, Handler: p.retransmissionQueue.AckHandler(protocol.Encryption1RTT)},
			)
	...
	if hasData {
		...
		pl.frames, pl.streamFrames, lengthAdded = p.framer.Append(
			pl.frames,
			pl.streamFrames,
			maxPayloadSize-pl.length,
			now,
			v,
		)
...
func (p *packetPacker) packPTOProbePacket1RTT(
	maxPacketSize protocol.ByteCount,
	addPingIfEmpty bool,
	now monotime.Time,
	v protocol.Version,
...
func (p *packetPacker) PackMTUProbePacket(
	ping ackhandler.Frame,
	size protocol.ByteCount,
	v protocol.Version,
) (shortHeaderPacket, *packetBuffer, error) {
...
func (p *packetPacker) PackPathProbePacket(
	...
	padding := protocol.MinInitialPacketSize - p.shortHeaderPacketLength(
		connID,
		pnLen,
		payload,
	) - protocol.ByteCount(
	...
	packet, err := p.appendShortHeaderPacket(
		buf,
		connID,
		pn,
		pnLen,
...
func (p *packetPacker) appendLongHeaderPacket(
	...
	if pn := p.pnManager.PopPacketNumber(encLevel); pn != header.PacketNumber {
		return nil, fmt.Errorf(
			"packetPacker BUG: Peeked and Popped packet numbers do not match: expected %d, got %d",
			pn,
			header.PacketNumber,
		)
	}
...
func (p *packetPacker) appendShortHeaderPacket(
	...
	if !isMTUProbePacket {
		if size := protocol.ByteCount(len(raw) + sealer.Overhead()); size > maxPacketSize {
			return shortHeaderPacket{}, fmt.Errorf(
...
func (p *packetPacker) appendPacketPayload(
	raw []byte,
	pl payload,
	...
	if payloadSize := protocol.ByteCount(len(raw)-payloadOffset) - paddingLen; payloadSize != pl.length {
		return nil, fmt.Errorf(
			"PacketPacker BUG: payload size inconsistent (expected %d, got %d bytes)",
			pl.length,
			payloadSize,
		)
	}
...
func (p *packetPacker) encryptPacket(
	raw []byte,
	sealer sealer,
	pn protocol.PacketNumber,
	payloadOffset, pnLen protocol.ByteCount,
) []byte {
