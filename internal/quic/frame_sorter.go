...
func (s *frameSorter) flushSeqToQueue() {
	if s.queue == nil {
		s.queue = make(map[protocol.ByteCount]frameSorterEntry, s.seqCount+4)
	}

	for s.seqCount > 0 {
		item := s.seqBuf[s.seqHead]
		s.queue[item.offset] = item.entry
		s.seqHead = (s.seqHead + 1) % len(s.seqBuf)
		s.seqCount--
	}
}
...
func (s *frameSorter) push(data []byte, offset protocol.ByteCount, doneCb func()) error {
	...
	// If there are no out-of-order gaps (s.gaps.Len() == 1), no reordered queue entries (len(s.queue) == 0),
	// and the frame offset matches the contiguous end of received data (s.gaps.Front().Value.Start == offset),
	// and there is room in the sequential ring buffer:
	if len(s.queue) == 0 && s.gaps.Len() == 1 && s.gaps.Front().Value.Start == offset {
		if s.seqCount < len(s.seqBuf) {
			s.gaps.Front().Value.Start = offset + protocol.ByteCount(len(data))
			idx := (s.seqHead + s.seqCount) % len(s.seqBuf)
			s.seqBuf[idx] = seqEntry{
				offset: offset,
				entry:  frameSorterEntry{Data: data, DoneCb: doneCb},
			}
			s.seqCount++
...
func (s *frameSorter) findEndGap(
	startGap *list.Element[byteInterval],
	offset protocol.ByteCount,
) (*list.Element[byteInterval], bool) {
...
func (s *frameSorter) Pop() (protocol.ByteCount, []byte, func()) {
	if s.seqCount > 0 && s.seqBuf[s.seqHead].offset == s.readPos {
		item := s.seqBuf[s.seqHead]
		s.seqHead = (s.seqHead + 1) % len(s.seqBuf)
		s.seqCount--
		s.readPos += protocol.ByteCount(len(item.entry.Data))

		return item.offset, item.entry.Data, item.entry.DoneCb
	}
...
func (s *frameSorter) HasMoreData() bool {
	return s.seqCount > 0 || len(s.queue) > 0
}
