...
func (u *packetUnpacker) UnpackShortHeader(
...
func (u *packetUnpacker) unpackShortHeader(
	hd headerDecryptor,
	data []byte,
) (int, protocol.PacketNumber, protocol.PacketNumberLen, protocol.KeyPhaseBit, error) {
...
func unpackLongHeader(hd headerDecryptor, hdr *wire.Header, data []byte) (*wire.ExtendedHeader, error) {
	...
	if protocol.ByteCount(len(data)) < hdrLen+4+16 {
		return nil, fmt.Errorf(
			"packet too small, expected at least 20 bytes after the header, got %d",
			protocol.ByteCount(len(data))-hdrLen,
		)
	}
