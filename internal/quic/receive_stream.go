...
func (s *ReceiveStream) readImpl(p []byte) (hasStreamWindowUpdate, hasConnWindowUpdate bool, _ int, _ error) {
	...
	for bytesRead < len(p) {
		...
		if bytesRead > len(p) {
			return hasStreamWindowUpdate, hasConnWindowUpdate, bytesRead, fmt.Errorf(
				"BUG: bytesRead (%d) > len(p) (%d) in stream.Read",
				bytesRead,
				len(p),
			)
		}
...
func (s *ReceiveStream) handleStreamFrameImpl(frame *wire.StreamFrame, now monotime.Time) error {
	//nolint:nilerr // Stream is already closed for shutdown, ignore incoming frame.
	if s.closeForShutdownErr != nil {
		return nil
	}
