...
func (c *streamFlowController) UpdateHighestReceived(offset protocol.ByteCount, final bool, now monotime.Time) error {
	...
	if c.receivedFinalOffset {
		...
		if offset > c.highestReceived {
			return &qerr.TransportError{
				ErrorCode: qerr.FinalSizeError,
				ErrorMessage: fmt.Sprintf(
					"received offset %d for stream %d, but final offset was already received at %d",
					offset,
					c.streamID,
					c.highestReceived,
				),
	...
	if offset < c.highestReceived {
		if final {
			return &qerr.TransportError{
				ErrorCode: qerr.FinalSizeError,
				ErrorMessage: fmt.Sprintf(
					"received final offset %d for stream %d, but already received offset %d before",
					offset,
					c.streamID,
					c.highestReceived,
				),
	...
	if c.checkFlowControlViolation() {
		return &qerr.TransportError{
			...
			ErrorMessage: fmt.Sprintf(
				"received %d bytes on stream %d, allowed %d bytes",
				offset,
				c.streamID,
				c.receiveWindow,
			),
...
func (c *streamFlowController) AddBytesRead(n protocol.ByteCount) (hasStreamWindowUpdate, hasConnWindowUpdate bool) {
	...
	return hasStreamWindowUpdate, hasConnWindowUpdate
}
