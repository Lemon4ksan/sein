...
func Register(app *sein.Server, prefix ...string) {
	...
	app.Get(p, adapt(pprof.Index))
	app.Get(p+"/", adapt(pprof.Index))
	app.Get(p+"/cmdline", adapt(pprof.Cmdline))
	app.Get(p+"/profile", adapt(pprof.Profile))
	app.Get(p+"/symbol", adapt(pprof.Symbol))
	app.Get(p+"/trace", adapt(pprof.Trace))
	app.Get(p+"/goroutine", adapt(pprof.Index))
	app.Get(p+"/heap", adapt(pprof.Index))
	app.Get(p+"/threadcreate", adapt(pprof.Index))
	app.Get(p+"/block", adapt(pprof.Index))
	app.Get(p+"/mutex", adapt(pprof.Index))
	app.Get(p+"/allocs", adapt(pprof.Index))
}
